...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  response.say('Please say your company name after the beep.');

  response.record({
    action: `/api/webhook/store-recording?replayId=${replayId}&callerId=${encodeURIComponent(
      callerId as string
    )}&field=companyRecordingUrl`,
    method: 'POST',
    maxLength: 10,
    timeout: 5,
    playBeep: true,
    trim: 'do-not-trim',
    recordingStatusCallback: `/api/webhook/record-phone?replayId=${replayId}&callerId=${encodeURIComponent(
      callerId as string
    )}`,
